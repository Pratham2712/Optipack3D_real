...
#         # Collect container details
#         # print(df)
#         numeric_columns = ['Gross Weight (in KGs)', 'Net Weight (in KGs)', 'Volume (in m^3)', 
#                            'Temperature (in deg  C)', 'Length (in mm)', 'Width (in mm)', 
#                            'Height (in mm)', 'Number of Cases']
        
#         df[numeric_columns] = df[numeric_columns].apply(pd.to_numeric, errors='coerce')
#         df_records = df.astype({col: np.float64 for col in numeric_columns}).to_dict(orient='records')

#         # Convert any numpy int64 or float64 to Python's int or float
...
#                 return obj.tolist()  # if you have numpy arrays
#             else:
#                 return obj
...
#         # print(threed_boxes)
#         # print(container_inf)
#         df_records = [{k: convert_numpy_types(v) for k, v in record.items()} for record in df_records]
#         context = {
#             'packaging_density': [int(x) for x in packd_list],
#             'vol_occ_curr': [round(float(x), 3) for x in vol_curr_list],
#             'vol_container':[round(float(x), 3) for x in vol_container_list],
#             'container_type' : container_type,
#             'container_indices' : list(range(1, num_containers + 1)),
